define([
    ...
    function TABLE() {
        ...
        function init(config, years, json, i18nLabels) {
            ...
            createHtmlTable(CONFIG.placeholder, years.reverse(), json);
            fillTable(json);
        ...
        function fillTable(json) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                // Insert Year value
                $("#" + CONFIG.prefix + "_" + row + "_" + json[i][index + 1]).html(formatValue(value));
        ...
        function addRow(row, serie, sumRow, totalValuesRow, first_column_value) {
            ...
            $("#" + CONFIG.prefix + "_avg_" + row + "").html(formatValue(Number(sumRow / totalValuesRow).toFixed(CONFIG.decimal_values)));
        ...
        function addTotals(columnsValues, totalAvg) {
            ...
            for (var year in columnsValues) {
                $("#" + CONFIG.prefix + "_total_" + year).html(formatValue(Number(columnsValues[year]).toFixed(CONFIG.decimal_values)));
            }
            $("#" + CONFIG.prefix + "_total_avg").html(formatValue(Number(totalAvg).toFixed(CONFIG.decimal_values)));
        ...
        function formatValue(x) {
            return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, CONFIG.thousand_separator);
        }
